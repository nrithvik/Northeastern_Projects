...
# Stop the http server on Replica Servers
for server in "${REPLICA_SERVER_LIST[@]}"
do
    ssh -i $keyfile $username@$server 'killall python > /dev/null 2>&1'
       ssh -i $keyfile $username@$server 'rm -rf ~/HTTP_JonRit_Test'

done

# Stop the dns server on cs5700cdnproject.com
...
ssh -i $keyfile $username@cs5700cdnproject.ccs.neu.edu 'killall dnsserver'
ssh -i $keyfile $username@cs5700cdnproject.ccs.neu.edu 'rm -rf ~/DNS_JonRit_Test'
